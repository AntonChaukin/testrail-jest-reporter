'use strict';
const chalk = require('chalk'), Api = require('./interface');
...
class Caller {
    ...
    constructor(_options) {
        this._baseUrl = _options && _options.baseUrl ;
        this._milestone_name = _options && _options.milestone;
        this._project_id = _options && _options.project_id;
        this._api = new Api({baseUrl: this._baseUrl});
    }
    ...
    add_results(testsResults) {
        ...
        return Promise.all(testsResults.map(run => this._api.add_results_for_cases(run.id, {"results": run.results} )))
            .then(responce => {
                let count = 0;
    ...
    get_tests() {
        return this._api.get_milestones(this._project_id, {is_completed: 0})
            .then(res => {
                ...
                if (!!_milestone.length) {
                    ...
                    return this._api.get_plans(this._project_id, {is_completed: 0, milestone_id: this._milestone_id});
                }
                return false;
            })
            .then(res => {
                if (!!res.length) {
                    return Promise.all(res.map(plan => this._api.get_plan(plan.id)));
                }
                return false;
            })
            .then(res => {
                for(let j=0; j<res.length; j++) {
                    const plan = res[j];
                    for (let i=0, len = plan.entries.length; i<len; i++) {
                        plan.entries[i].runs.map(run => this._runs_ids.push(run.id));
                    }
                }
                if (this._milestone_id) {
                    return this._api
                        .get_runs(this._project_id, {is_completed: 0, milestone_id: this._milestone_id});
                }
                return false;
            })
            .then(res => {
                ...
                if (!!this._runs_ids.length) {
                    return Promise.all(this._runs_ids.map(id => this._api.get_tests(id)));
                }
                return false;
            })
            .then(res => {
