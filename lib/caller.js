...
class Caller {
    ...
    add_results(testsResults) {
        ...
        return Promise.all(
            testsResults.map(run => this._api.add_results_for_cases(run.id, {"results": run.results}))
        )
            .then(response => {
                let count = 0;
        ...
        return this._api.get_milestones(this._project_id, {is_completed: 0})
            .then(res => {
                ...
                if (!!_milestone.length) {
                    ...
                    return this._api
                        .get_plans(this._project_id, {is_completed: 0, milestone_id: this._milestone_id});
                }
                return false;
            })
            .then(res => {
